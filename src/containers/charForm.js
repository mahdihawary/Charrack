  ...
  const displayActive = () => {
    switch (activeStep) {
      ...
      case 4:
        return (
          <div>
            <CharShow character={character} />
            <Paper>
              <Button onClick={createCharacter}>
                <NavLink to="/characters">Submit</NavLink>
              </Button>{" "}
            </Paper>
          </div>
        );
    ...
    <Grid container justify="center" spacing={0}>
      ...
      <Grid xs={3}>
        <MobileStepper
          ...
          nextButton={<Button size="small" disabled={true}></Button>}
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    resetCharacter: () => dispatch(resetChar()),
  };
};
